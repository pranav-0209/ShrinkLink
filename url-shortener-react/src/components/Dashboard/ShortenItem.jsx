...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
  ...
  const analyticsHandler = (shortUrl) => {
    ...
    const fetchMyShortUrl = async () => {
        setLoader(true);
        try {
             const { data } = await api.post(`/api/urls/analytics/${selectedUrl}?startDate=2024-12-01T00:00:00&endDate=2025-12-31T23:59:59`, {},{
                        headers: {
  ...
  return (
    ...
    <div className={`flex sm:flex-row flex-col  sm:justify-between w-full sm:gap-0 gap-5 py-5 `}>
      <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
        ...
        <div className="flex items-center gap-1 ">
          ...
          <div className="flex   items-center gap-8 pt-6 ">
            <div className="flex gap-1  items-center font-semibold  text-btnColor">
              <span>
      ...
      >
        <div role="button" aria-label="Copy short url" className="flex cursor-pointer gap-1 items-center bg-btnColor py-2 font-semibold shadow-md shadow-slate-500 px-6 rounded-md text-white">
      ...
      <button
        ...
        className="flex items-center gap-2 py-2 px-4 rounded-md bg-indigo-600 text-white font-semibold shadow-sm hover:bg-indigo-700 transition"
        aria-pressed={analyticToggle}
        type="button"
      >
        <span>Analytics</span>
